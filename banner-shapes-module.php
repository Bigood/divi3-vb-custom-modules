...
function mj_initialize_divi_modules() {

  if ( class_exists('ET_Builder_Module')) {

    class ET_Builder_Module_Fullwidth_Header_Shapes extends ET_Builder_Module {
      function init() {
        $this->name             = esc_html__( 'Bannière triangle', 'et_builder' );
        $this->slug             = 'mj_banniere_shapes';
      ...
      function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        if ( '' !== $background_overlay_color ) {
          ET_Builder_Element::set_style( $function_name, array(
            'selector'    => '%%order_class%%.et_pb_fullwidth_header .triangle-second',
            'declaration' => sprintf(
              'fill: %1$s;',
              esc_html( $background_overlay_color )
            ),
        ...
        //Bigood : change
        $output = sprintf(
          '<section%9$s class="custom_header_shapes et_pb_fullwidth_header%1$s%7$s%8$s%10$s%11$s">
            %6$s
            %12$s
            <div class="shapes">
    ...
    // $et_pb_fullwidth_header_shapes = new ET_Builder_Module_Fullwidth_Header_Shapes();
    // add_shortcode( 'et_pb_fullwidth_header_shapes', array($et_pb_fullwidth_header_shapes, '_shortcode_callback') );
    new ET_Builder_Module_Fullwidth_Header_Shapes;
  }

}
// add_action('et_builder_ready', 'ex_divi_child_theme_setup');
add_action( 'et_builder_ready', 'mj_initialize_divi_modules' );
